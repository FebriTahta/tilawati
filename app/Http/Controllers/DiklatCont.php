...
class DiklatCont extends Controller
{
    ...
    public function data_forward_konfirm_cabang(Request $request, $pelatihan_id)
    {
        if(request()->ajax())
        {
            ...
            ->addColumn('action', function($data){
                $remove = '<a href="/remove-share-forward/'.$data->id.'" class="btn btn-sm btn-outline btn-warning" ><i class="fa fa-trash"></i></a>';
                return $remove;
            })
            ...
            if(!empty($request->dari))
            {
                ...
                return DataTables::of($data)
                        ...
                        ->addColumn('action', function($data){
                            ...
                            $actionBtn .= ' <button data-id="'.$data->id.'" alt="cetak data peserta" class="btn btn-sm btn-info" data-toggle="modal" data-target="#modal-download"><i class="fa fa-download"></i></button>';
                            // $actionBtn .= ' <button data-id="'.$data->id.'" alt="cetak surat jalan" class="btn btn-sm btn-success" data-toggle="modal" data-target="#modal-download2"><i class="fa fa-download"></i></button>';
                            $actionBtn .= ' <a href="#" class="btn btn-sm btn-outline btn-success" data-slug="'.$data->slug.'" data-nama_diklat="'.$data->program->name.'" data-id="'.asset('images/'.$data->slug.'.png').'" data-toggle="modal" data-target=".modal-scan"><i class="mdi mdi-barcode-scan"></i></a>';
                            return $actionBtn;
    ...
    public function diklat_peserta_cabang_total(Request $request, $cabang_id)
    {
        if (request()->ajax()) {
            # code...
            if(!empty($request->dari))
            {
                $data1 = Peserta::where('cabang_id', $cabang_id)->whereBetween('tanggal', array($request->dari, $request->sampai))->whereHas('program',function($data){
                    $data->where('jenisprogram','santri');
                })->count();

                $data2 = Peserta::where('cabang_id', $cabang_id)->whereBetween('tanggal', array($request->dari, $request->sampai))->whereHas('program',function($data){
                    $data->where('jenisprogram','guru');
                })->count();
                $data = 'santri : '. $data1.' & guru : '. $data2;
                return response()->json($data,200);
            }
            else
            {
                $data1 = Peserta::where('cabang_id', $cabang_id)->whereHas('program',function($data){
                    $data->where('jenisprogram','santri');
                })->count();

                $data2 = Peserta::where('cabang_id', $cabang_id)->whereHas('program',function($data){
                    $data->where('jenisprogram','guru');
                })->count();
                $data = 'santri : '. $data1.' & guru : '. $data2;
                return response()->json($data,200);
