...
class SyahadahCont extends Controller
{
    ...
    public function syahadah_cabang($cabang_id, Request $request)
    {
        if ($request->ajax()) {
            # code...
            if(!empty($request->dari))
            {
                $data   = Pelatihan::where('cabang_id', $cabang_id)->with('cabang','program')->withCount('peserta')->orderBy('id','desc')->where('jenis','diklat')->where('syahadah','1')
    ...
    public function syahadah_terbit_cabang($cabang_id, Request $request)
    {
        if ($request->ajax()) {
            # code...
            if(!empty($request->dari))
            {
                # code...
                $terbit = Pelatihan::where('cabang_id', $cabang_id)->where('syahadah','1')->whereBetween('updated_at', array($request->dari, $request->sampai))->count();
                $peserta_terbit = Peserta::where('cabang_id', $cabang_id)->where('bersyahadah','1')->whereHas('pelatihan', function($q) use ($request){
                    $q->where('syahadah','1')->whereBetween('updated_at', array($request->dari, $request->sampai));
                })->count();
                ...
                # code...
                $terbit = Pelatihan::where('cabang_id', $cabang_id)->where('syahadah','1')->count();
                $peserta_terbit = Peserta::where('cabang_id', $cabang_id)->where('bersyahadah','1')->whereHas('pelatihan', function($q){$q->where('syahadah','1');})->count();
