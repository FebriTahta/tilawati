...
class PesertaCont extends Controller
{
            ...
            $data   = Peserta::where('pelatihan_id', $id)->with('certificate')->with('pelatihan')->with('kabupaten')->with('nilai')->where('status',1)->orderBy('id','asc');
                return DataTables::of($data)
                        ...
                        ->addColumn('action', function($data){
                            ...
                            $actionBtn .= ' <a href="#" data-toggle="modal" data-target="#modal-edit" class="btn btn-sm btn-outline btn-primary "><i class="fa fa-pencil-square"></i></a>';
                            return $actionBtn;
                        })
    ...
    public function peserta_program_data(Request $request, $program_id)
    {
        if(request()->ajax())
        {
            if(!empty($request->dari))
            {
                $data   = Peserta::where('program_id', $program_id)->whereBetween('tanggal', array($request->dari, $request->sampai))->with('pelatihan')->with('kabupaten')->with('nilai');
                return DataTables::of($data)
                        ->addColumn('nilai', function ($data) {
                            if ($data->nilai->count() == 0) {
                                # code...
                ...
                $data   = Peserta::where('program_id', $program_id)->with('pelatihan')->with('kabupaten')->with('nilai');
                return DataTables::of($data)
                        ->addColumn('nilai', function ($data) {
                            if ($data->nilai->count() == 0) {
                                # code...
